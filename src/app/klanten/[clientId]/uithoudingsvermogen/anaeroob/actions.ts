...
function fields(formData: FormData) {
  return {
    ...
    quebec_10s_watt: numOrNull(formData.get("quebec_10s_watt")),
    vertical_jump_cm: numOrNull(formData.get("vertical_jump_cm")),
    wingate_watt: numOrNull(formData.get("wingate_watt")),
  };
}

export async function addAnaerobeTest(clientId: string, formData: FormData) {
  const supabase = await createClient();
  const photoPath = await uploadPhotoIfPresent(supabase, clientId, formData);

  const { error } = await supabase.from("anaerobe_testen").insert({
    client_id: clientId,
    ...fields(formData),
    photo_path: photoPath,
  });
  if (error) throw new Error(error.message);
  revalidatePath(`/klanten/${clientId}/uithoudingsvermogen/anaeroob`);
}

export async function updateAnaerobeTest(id: string, formData: FormData) {
  const supabase = await createClient();
...
export async function deleteAnaerobeTest(id: string) {
  const supabase = await createClient();
